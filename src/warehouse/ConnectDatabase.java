...
public class ConnectDatabase {
	...
	public ResultSet loadDBConfig(int idConfig) throws SQLException, ClassNotFoundException {
		...
		sql = "Select * from Config where idconfig="+String.valueOf(idConfig);
	...
	public ResultSet loadStagingDB(int idConfig) throws SQLException, ClassNotFoundException {
		...
		Connection connection;
		ResultSet result = null;
		ResultSet config=loadDBConfig(idConfig);
		config.next();
			String serverName = config.getString("SVNameSta");
			String databaseName = config.getString("DBNameSta");
			String sourceTB = config.getString("TBNameSta");
			String user = config.getString("UserNameSta");
			String pass = config.getString("PassSta");
	...
	public Connection connectWarehouseDB(int idConfig) throws SQLException, ClassNotFoundException {
		...
		ResultSet config=loadDBConfig(idConfig);
		config.next();
		String serverName = config.getString("SVNameWH");
		String databaseName = config.getString("DBNameWH");
		String user = config.getString("UserNameWH");
		String pass = config.getString("PassWH");
			Class.forName("com.mysql.jdbc.Driver");
			String connectionURL = "jdbc:mysql://" + serverName + "/" + databaseName
					+ "?useUnicode=true&amp;characterEncoding=utf8?autoReconnect=true&useSSL=false";
